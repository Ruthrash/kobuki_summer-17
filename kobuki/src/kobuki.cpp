...
{ 
  ...
  //cout<<"The goal is "<<tg<<"\n";
  t_error = tg - tc;
  float t_kp = 3, t_ki = 0.0, t_kd = 0.0;//setting PID gains
  float  omega = 0.0; 
...
int main(int argc, char **argv)
{
	...
	while(ros::ok()) //while ros is running properly
	{
		...
		if(abs(xg-xc) > 0.05 && abs(yg-yc) > 0.05)
			v = 0.05;
	    else 
			v = 0.0;
